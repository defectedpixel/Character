...
//attempt to introduce sound oh my god
		const sfxExt = SFX.supportsAudioType('mp3') ? 'mp3' : 'ogg';

		const game = this;
		this.anims = ["injuredwalking", "jump", "lookaround"];
	...
	init() {
		...
		loader.load( `${this.assetsPath}fbx/man.fbx`, function ( object ) {
	...
	playerControl(forward, turn){
		...
		if (forward>0){
			if (this.player.action!='injuredwalking') this.action = 'injuredwalking';
		}else{
			if (this.player.action=="injuredwalking") this.action = 'lookaround';
		}
		if (forward==0 && turn==0){
...
class SFX{
	constructor(options){
		...
		if (codec!=undefined){
			...
			console.warn("Browser does not the audio my duuudeeeee");
		}
	}
